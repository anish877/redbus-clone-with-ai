import { Cctv, CirclePlus, Cookie, Lamp, MapPin, Milk, PlugZap, Star, Ticket } from 'lucide-react'
...
const BusTicketDetailsCard = ({
  ...
  return (
    ...
    >
      <div className='grid grid-cols-10 w-full justify-center'>
        <div className='col-span-3 flex flex-col px-2 py-4 text-sm gap-y-6'>
        ...
        <div className='col-span-1 flex flex-col text-sm gap-y-5 py-3'>
          <p className='text-xl font-bold p-0 m-0'>{departureTime}</p>
          <p className='font-light text-gray-500 text-xs'>{destination}</p>
        </div>
        <div className='col-span-1 flex flex-col px-2 py-4 text-sm gap-y-6 text-gray-500 font-light'>
          <p>{durationTime}</p>
        </div>
        <div className='col-span-1 flex flex-col text-sm gap-y-5 py-3'>
        ...
        <div className='col-span-1 flex flex-col py-4 text-sm gap-y-6'>
        ...
        <div className='col-span-1 flex flex-col py-3 items-center font-extralight gap-0 text-left'>
          <p>Starts from
          INR <span className='font-bold text-lg'>{fare}</span></p>
        </div>
        <div className='col-span-2 flex justify-center py-8 text-gray-500 font-light text-sm'>
          ...
          <div className='flex'>
            <div className='overflow-hidden transition-[width] duration-300 ease-in-out' style={{ width: showButtons ? 'auto' : '0' }}>
              <div className='whitespace-nowrap'>
                <button 
                  className={`py px-3 font-extralight text-sm pl-5 border-r border-red-600 hover:text-red-600 ${activeSection === 'amenities' ? "font-extrabold underline text-red-600" : ""}`}
                  onClick={() => handleSectionClick('amenities')}
                >
                  Amenities
                </button>
                <button 
                  className={`py px-3 font-extralight text-sm pl-5 border-r border-red-600 hover:text-red-600 ${activeSection === 'boarding' ? "font-extrabold underline text-red-600" : ""}`}
                  onClick={() => handleSectionClick('boarding')}
                >
                  Boarding & Dropping Points
                </button>
                <button 
                  className={`py px-3 font-extralight text-sm pl-5 hover:text-red-600 ${activeSection === 'reviews' ? "font-extrabold underline text-red-600" : ""}`}
            ...
            <button className='py-1.5 px-3 bg-red-600 text-white text-sm'>VIEW SEATS</button>
          </div>
        </div>
      </div>

      {activeSection === 'amenities' && (
        <div className='bg-[#f8f9fa] p-16'>
          <div className='grid-cols-3 grid text-sm gap-y-3 justify-center items-center'>
      ...
      {activeSection === 'boarding' && (
        <div className='bg-[#f8f9fa] p-12 flex justify-evenly pl-5'>
